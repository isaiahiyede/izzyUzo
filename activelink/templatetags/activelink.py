...
'''Added by AJ for Django 1.9> support'''
try:
...
#custom class added by Abiodun Ajibike on Dec 06 2015
class ActiveLinkNotEqualNode(ActiveLinkNodeBase):
...
#custom function added by Abiodun Ajibike on Dec 06 2015
@register.tag
def ifnotactive(parser, token):
    urlnode = url(parser, token)
    var, nodelist_true, nodelist_false = parse(parser, token, 'endifnotactive')
    return ActiveLinkNotEqualNode(urlnode, var, nodelist_true, nodelist_false)
